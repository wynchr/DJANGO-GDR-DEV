...
.DESCRIPTION
    ...
    Create Oracle Table (SA.REFID_IOP_PROVLOG_AD)
    Copy from PS
...
.COMMENTS
    CREATE TABLE SA.REFID_IOP_PROVLOG_AD (
            ...
            OBJECTSID					CHAR(50), 
            OBJECTGUID					CHAR(50), 
            CONSTRAINT REFID_IOP_PROV_AD_ID_PK PRIMARY KEY (ID)
       )
=======================================================================================================================
"""
# =========================================
...
def Get_REFID_IOP_PROVLOG_AD():
    sql_query = "select * from SA.REFID_IOP_PROVLOG_AD"
    dataset = oracle_IT.fetch_data_from_db(sql_query)
    return dataset


def Drop_REFID_IOP_PROVLOG_AD():
    sql_query = "DROP TABLE SA.REFID_IOP_PROVLOG_AD"
    dataset = oracle_IT.exec_ddl_sql(sql_query)
    return dataset


def Create_REFID_IOP_PROVLOG_AD():
    sql_query = f"""
    CREATE TABLE SA.REFID_IOP_PROVLOG_AD (
            ID 							NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,
            SAMACCOUNTNAME 				VARCHAR2(10), 
        ...
        CONSTRAINT REFID_IOP_PROVLOG_AD_ID_PK PRIMARY KEY (ID)
    )
    """
    dataset = oracle_IT.exec_ddl_sql(sql_query)
    return dataset
...
def CreateIndex_REFID_IOP_PROVLOG_AD():
    sql_query = "CREATE INDEX REFID_IOP_PROVLOG_AD_SAMACCOUNTNAME_PK ON SA.REFID_IOP_PROVLOG_AD (SAMACCOUNTNAME)"
    dataset = oracle_IT.exec_ddl_sql(sql_query)
    return dataset
...
logging.info(f"Create Oracle Table (SA.REFID_IOP_PROVLOG_AD) for DB:{DB}")

try:
    logging.info(f"Get_REFID_IOP_PROVLOG_AD")
    dataset = Get_REFID_IOP_PROVLOG_AD()
    logging.info(f"{len(dataset)} rows")
except:
    logging.info(f"Table do not exist")

try:
    logging.info(f"Drop_REFID_IOP_PROVLOG_AD")
    dataset = Drop_REFID_IOP_PROVLOG_AD()
    logging.info(f"{dataset}")
except:
    logging.info(f"Table do not exist")

try:
    logging.info(f"Create_REFID_IOP_PROVLOG_AD")
    dataset = Create_REFID_IOP_PROVLOG_AD()
    logging.info(f"{dataset}")
except:
    logging.info(f"Cannot create table")

try:
    logging.info(f"CreateIndex_REFID_IOP_PROVLOG_AD")
    dataset = CreateIndex_REFID_IOP_PROVLOG_AD()
    logging.info(f"{dataset}")
except:
    logging.info(f"Cannot create index table")
